...
function Backoff(workers, options) {
  ...
  function updateMaxTasks(newMaxTasks, reason) {
    ...
    debug(
      'setting maxTasks [new=%s, old=%s, reason="%s"]',
      newMaxTasks, currentMaxTasks, reason
    );
  }
  ...
  workers.on('executeTask', function() {
    ...
    if (maxActiveTasks !== -1 && activeTasks >= maxActiveTasks) {
      updateMaxTasks(0, 'task started');
      ...
      if (pollingInterval !== defaultPollingInterval) {
        return updatePollingInterval(defaultPollingInterval, 'max tasks reached');
      }
    }
  });
  ...
  workers.on('executeTask:done', function() {
    ...
    if (maxActiveTasks !== -1 && activeTasks < maxActiveTasks) {
      return updateMaxTasks(defaultMaxTasks, 'task completed');
    }
  });
