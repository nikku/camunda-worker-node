...
Workers.prototype.executeTask = function(task) {
  ...
  var taskContext = extend({}, task, {
    variables: deserializedVariables,
    extendLock: function() {

      var argCount = arguments.length;

      var newDuration = arguments[0];

      if (argCount === 1) {
        // promise based invocation
        return new Promise(function(resolve, reject) {
          self.extendLock(task, newDuration, function(err) {
      ...
      if (argCount === 2) {
        ...
        return self.extendLock(task, newDuration, arguments[1]);
      }

      throw new Error(
        'expected Promise or callback style ' +
        'invocation with args <newDuration>'
      );
    }
  });
  ...
  workerDefinition.fn(taskContext, function(err, newContext) {
    ...
    if (err) {
      ...
      self.emit('task:complete', task, newContext);

      newContext = newContext || {};

      errorCode = newContext.errorCode;
      ...
      if (errorCode) {
        ...
        newVariables = newContext.variables || {};
      ...
      self.engineApi[errorCode ? 'bpmnError' : 'taskCompleted'](task.id, body, function(err) {
        if (err) {
 ...
 * @param {Function} callback
 */
Workers.prototype.extendLock = function(task, newDuration, callback) {
  this.emit('task:extendLock', task, newDuration);

  var self = this;

  return this.engineApi.extendLock(task.id, {
    workerId: this.options.workerId,
    newDuration: newDuration
  }, function(err) {

    if (err) {
      self.emit('task:extendLock:fail', task, newDuration, err);
    } else {
      self.emit('task:extendLock:success', task, newDuration);
    }
