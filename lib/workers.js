...
Workers.prototype.poll = async function() {
  ...
  try {
    ...
    let tasks = this.lastTasks = await this.fetchTasks(topics);
...
Workers.prototype.start = function() {
  ...
  var schedulePoll = () => {
    ...
    var waitMs = pollingInterval;

    var lastTasks = this.lastTasks;
    var lastPoll = this.lastPoll;

    if (lastTasks.length === maxTasks) {
      waitMs = 0;
    } else {
      waitMs = Math.max(0, waitMs - (now() - lastPoll));
    }

    this.reschedule(waitMs);
  };
