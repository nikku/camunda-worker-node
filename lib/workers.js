...
function parseOptions(options) {
  return extend({workerId: "customWorker"}, defaultOptions, options || {});
}
...
Workers.prototype.shutdown = function(callback) {
  ...
  if (this.boundReschedule) {
    this.removeListener('poll:complete', this.boundReschedule);
  }
...
Workers.prototype.start = function start() {
  ...
  this.boundReschedule = this.reschedule.bind(this);
  this.on('poll:complete', this.boundReschedule);
 ...
 */
Workers.prototype.enableLogging = function enableLogging() {
  this.on('start', consumerId =>
