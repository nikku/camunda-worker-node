...
function Workers(baseUrl, opts) {
  ...
  // apply extensions
  if (opts.use) {
    this.extend(opts.use);
  }

  // start
  if (opts.pollingDelay !== -1) {
    this.start();
  }
}
...
Workers.prototype.poll = async function() {
  ...
  try {
    ...
    if (!topics.length) {
      this.emit('poll:done', 'no-topics', now() - this.lastPoll);
    ...
    if (!tasks.length) {
      this.emit('poll:done', 'no-tasks', now() - this.lastPoll);
  ...
  this.emit('poll:done', 'processed', now() - this.lastPoll);
};
...
Workers.prototype.executeTasks = async function(tasks) {
  ...
  await Promise.all(
    tasks.map(
      t => this.executeTask(t)
    )
  );

  this.emit('executeTasks:done', now() - execStart);
};
...
Workers.prototype.start = function() {
  ...
  var schedulePoll = () => {
    ...
    var diff = now() - this.lastPoll;

    this.reschedule(pollingInterval - diff);
  };
