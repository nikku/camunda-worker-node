...
function parseOptions(options) {
  return extend({ consumerId: uuid.v4() }, defaultOptions, options || {});
}
...
Workers.prototype.poll = function() {
  ...
  this.engineApi.multiPoll({
    ...
    if (err) {
      ...
      self.emit('poll:success', result.tasks);

      result.tasks.forEach(self.executeTask, self);
    }
...
Workers.prototype.shutdown = function(callback) {
  this.emit('shutdown', this.options.consumerId);

  this.removeListener('poll:success', this.reschedule);
...
Workers.prototype.start = function start() {
  this.emit('start', this.options.consumerId);

  this.on('poll:complete', this.reschedule.bind(this));
