...
function Logger(workers) {
  ...
  workers.on('task:extendLock', function(task, newDuration) {
    debug('extending task lock', task.id, newDuration);
  });

  workers.on('task:extendLock:success', function(task, newDuration) {
    debug('task lock extended', task.id, newDuration);
  });

  workers.on('task:extendLock:failed', function(task, newDuration, err) {
    debug('failed to extend task lock', task.id, newDuration, err);
  });
